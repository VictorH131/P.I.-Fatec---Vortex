<?php
    ...
    if($_SESSION['class'] == "aluno"){
        ...
        if (isset($_POST['email'], $_POST['senha'], $_POST['csrf_token'])) {
            ...
            try {
                $sql = "SELECT id_aluno, nome, senha, matricula FROM aluno WHERE email_institucional = ? LIMIT 1";
        ...
        if (isset($_POST['email'], $_POST['senha'], $_POST['csrf_token'])) {
            ...
            if ($_SESSION['login_attempts'] >= 7) {
                if (time() - $_SESSION['last_attempt'] < 180) { // 3 minutos
                    ...
                    $_SESSION['login_attempts'] = 0; // Zera após o tempo
                }
            }
            ...
            // VERIFICAÇÃO NO BANCO
            try {
                $sql = "SELECT id_adm, nome, senha, matricula_docente FROM adm WHERE email = ? LIMIT 1";
                $stmt = $conn->prepare($sql);
                $stmt->execute([$email]);
